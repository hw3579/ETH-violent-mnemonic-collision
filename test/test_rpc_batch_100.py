...
# 测试地址列表 (一些随机的以太坊地址 - 扩展到100个以支持大批量测试)
TEST_ADDRESSES = [
...
def test_batch_request(rpc_url, addresses, batch_size=5):
    ...
    try:
        start_time = time.time()
        response = requests.post(rpc_url, json=batch_payload, headers=headers, timeout=15)  # 增加超时时间
        elapsed = time.time() - start_time
...
def test_rpc_node(rpc_url):
    ...
    # 测试不同大小的批量请求 - 增加100的测试
    batch_sizes = [2, 5, 10, 20, 50, 100]
    max_batch = 0
    batch_success = False
    batch_time = 0
    batch_100_success = False
    ...
    if batch_success:
        ...
        if batch_100_success:
            print(f"{green}该节点支持100个批量请求!{reset}")
    else:
...
def main():
    ...
    batch_100_supported = [r for r in results if r["batch_100"]]
    ...
    print(f"{green}支持100批量请求的节点数: {len(batch_100_supported)}{reset}")
    ...
    if batch_100_supported:
        print(f"\n{yellow}支持100批量请求的节点:{reset}")
        for node in batch_100_supported:
            print(f"{green}- {node['url']} (性能提升: {(node['single_time']*100/node['batch_time']):.2f}倍){reset}")
        
        # 输出到专门的文件
        with open("batch_100_rpc_nodes.txt", "w", encoding="utf-8") as f:
            for node in batch_100_supported:
                f.write(f"{node['url']}\n")
        
        print(f"\n{green}已将支持100批量请求的节点保存到 batch_100_rpc_nodes.txt{reset}")
